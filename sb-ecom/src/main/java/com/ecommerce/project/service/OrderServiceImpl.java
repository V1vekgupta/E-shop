...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDTO updateOrder(Long orderId, String status) {
        ...
        // Seller can only update orders that contain at least one of their products
        if (authUtil.hasRole(AppRole.ROLE_SELLER)) {
            User seller = authUtil.loggedInUser();
            boolean allowed = order.getOrderItems().stream().anyMatch(orderItem -> {
            ...
            if (!allowed) {
                throw new APIException("You are not allowed to update this order");
            }
        }
